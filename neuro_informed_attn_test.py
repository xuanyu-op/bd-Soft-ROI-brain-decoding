...
class MaskRegistered(nn.Module):
    """
    Load and register the visual masks and affine matrices for each subject.
    By default, subjects 1, 2, 5, and 7 are supported.
    """
...
class NeuroscienceInformedAttentionLayer(nn.Module):
    ...
    def forward(self, values: torch.Tensor, keys: torch.Tensor, context=None):
        """
        Use learnable queries to compute attention scores over the keys,
        and then perform weighted aggregation over the voxel scalar values.
        """
